...
pd.concat([df_france, df_italy], axis=0)
```
...
```{code-cell} ipython3
clean_df = df_final.pivot_table(index=["country","date"], values=["confirmed", "deaths", "recovered"])
clean_df
```
...
```{code-cell} ipython3
clean_df2.loc["France","confirmed":"deaths"].plot()
```
...
```{code-cell} ipython3
df3 = clean_df.loc[(["France", "Italy", "Germany"],time_slice), "confirmed":"deaths"]
df3
```
...
df_cartesian = df3.unstack(level=0) 
df_cartesian
```
...
def extract(pays, mesures, dates):
    time = slice(dates[0], dates[1])

    
    return clean_df.loc[(pays,time), mesures].unstack(level=0)
...
dates = ["2021-07-01", "2021-08-14"]
extract(pays, mesures, dates).plot()
```

## proposez des analyses personnelles sur ces données

```{code-cell} ipython3
#Le nombre de morts étant bien plus faible que le nombre de cas confirmés; on a du mal à différencier les courbes des morts 
```

```{code-cell} ipython3
extract(pays, ["deaths"], dates).plot()
```

```{code-cell} ipython3
#On remarque que la France a plus de cas confirmé et moins de morts, peut étre du à une meilleure gestion de lépidémie (merci Macron ?)
```
